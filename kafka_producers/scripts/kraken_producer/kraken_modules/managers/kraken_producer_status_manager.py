...
class KrakenProducerStatusManager:
    ...
    async def register_component(self, component_name: str, new_component: KrakenBaseComponent):
        """새로운 상태 관리 컴포넌트 등록"""
        try:
            ...
            self.logger.exception_common(error=e, description=f"{component_name} 스테이터스 관리 대상 등록")
    ...
    async def refresh_one_component_statuses(self, status_managed_component_name: str):
        ...
        """
        await self._status_map[status_managed_component_name].component.update_component_status()
    ...
    async def refresh_all_component_statuses(self):
        ...
        try:
            ...
            self.logger.exception_common(error=e, description=f"{name} 컴포넌트 헬스 체크 상태 갱신")
            # 그대로 raise
            raise
