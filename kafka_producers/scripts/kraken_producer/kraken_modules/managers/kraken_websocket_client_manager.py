...
from kraken_modules.config_models import (
    ...
    KrakenWebSocketClientManagerConfigModel,
)
...
class KrakenWebSocketClientManager(KrakenBaseComponent):
    ...
    async def initialize_clients_manager(
        self, config_list: List[KrakenBaseWebSocketClientConfigModel]
    ):
    ...
    async def update_component_status(self) -> bool:
        ...
        await self.status_manager.update_manager_status(
            component_name=self.component_name, new_status=self.check_component_health()
        )
    ...
    async def reload_symbols(self):
        ...
        (
            new_total_symbols,
            new_subscription_symbols,
            new_unsubscription_symbols,
        ) = await self.active_pair_manager.refresh()
        self.logger.logger.info(
            f"새로운 심볼: {len(new_total_symbols)} 개\n새로운 구독 심볼: {len(new_subscription_symbols)} 개\n새로운 구독 취소 심볼: {len(new_unsubscription_symbols)} 개"
        )
        ...
        try:
            for client in self.clients.values():
                ...
                self.logger.info_success(
                    f"{client.config.component_name} 구독 심볼 갱신"
                )
