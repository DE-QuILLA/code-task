...
class KrakenRedisClient(KrakenBaseComponent):
    ...
    def __init__(
        self,
        config: KrakenRedisClientConfigModel,
    ...
    async def _ping_with_retry(self, desc_override: str = None) -> bool:
        ...
        try:
            ...
            raise KrakenProdcuerRedisConnectionException(
                f"[{description}] 도중 Redis ping 실패"
            )
    ...
    async def fetch_all_data(self, redis_key: str):
        ...
        try:
            ...
            raise KrakenProdcuerRedisFetchDataException(
                f"{redis_key} 키 데이터 fetch 실패"
            )
    ...
    async def check_component_health(self) -> KrakenProducerComponentHealthStatus:
        ...
        try:
            ...
            pong = await self._ping_with_retry(
                desc_override="헬스 체크 - Redis 연결 상태 확인"
            )
            if pong:
                ...
                self.logger.warning_common(
                    description="헬스 체크 중 예외 없는 Redis Ping 실패"
                )
            return status
        except Exception as e:
