...
# config 모델
from kraken_modules.config_models import KrakenStandardLoggerConfigModel, KrakenRedisClientConfigModel, KrakenActivePairManagerConfigModel, KrakenKafkaClientConfigModel, ALL_WEBSOCKET_CLIENT_CONFIG_MODELS

# 내부 활용 객체
from kraken_modules.logging import KrakenStdandardLogger
from kraken_modules.managers import KrakenProducerStatusManager, KrakenActiveSymbolManager, KrakenWebSocketClientManager
from kraken_modules.clients import KrakenKafkaClient, KrakenWebSocketClient, KrakenRedisClient
...
async def lifespan(kraken_fast_api_app: FastAPI, api_level_logger: KrakenStdandardLogger):
    try:
        ...
        active_pair_manager = KrakenActiveSymbolManager(redis_client=redis_client, config=active_pair_manager_config,)
